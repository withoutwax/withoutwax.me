  ...
  return (
    <>
      <div className="flex flex-col md:flex-row mx-auto bg-white dark:bg-black md:bg-transparent w-full max-w-[900px]">
        <nav className="navbar z-10 bg-white dark:bg-black bg-opacity-50 sticky top-0 flex md:flex-col flex-wrap gap-8 md:gap-0 justify-between md:h-screen items-center md:items-end px-8 py-8 md:py-16 my-0 w-full">
          <div className="order-1 md:order-0 flex justify-between w-full md:flex-col">
            ...
            <NextLink href="/contact">
              <a className="order-1 md:order-0 text-xl p-1 sm:p-4 text-gray-900 dark:text-gray-100">
          ...
          <div className="hidden order-0 md:order-1 md:flex md:flex-col items-center">
            <DarkModeButton theme={theme} setTheme={setTheme} />
          </div>
        </nav>
        <main className="flex flex-col w-full bg-white dark:bg-black p-8 md:mt-8 mb-8">
          {children}
          <Footer />
        </main>
      </div>
